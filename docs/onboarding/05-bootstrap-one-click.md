...
Use this guide when you want the repository creation, policy rendering, AWS foundation setup, and stack bootstrap to run in one command.

如果你希望通过一个命令完成仓库创建、策略生成、AWS 基础设施初始化和 stack bootstrap，请使用本文档。
...
8. Confirm the `devo` and `prod` Pulumi stacks were initialized.
...
8. 确认 `devo` 和 `prod` Pulumi stack 已初始化。
...
- `bootstrap-deployment-repo.sh --stack devo`
- `bootstrap-deployment-repo.sh --stack prod`

## Expected Result / 预期结果

You finish with repository configuration written to GitHub and Pulumi stacks initialized for both environments.

执行完成后，你应该获得已经写入 GitHub 的仓库配置，以及为两个环境初始化好的 Pulumi stack。
