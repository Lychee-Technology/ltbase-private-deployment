...
11. 检查可选的应用 override（未设置时自动使用默认值）：
    ...
    - `LTBASE_LTSEARCH_MODE`、`LTBASE_CDC_MODE`、`LTBASE_LTFLOW_MODE`、`LTBASE_SEMANTIC_MODE`、`LTBASE_GOVERNANCE_MODE`（默认 `auto`）
    - `LTBASE_GOVERNANCE_ACTION_MODE`（默认 `off`）
    - 依赖链：`LTBASE_GOVERNANCE_MODE=on` 要求 `LTBASE_SEMANTIC_MODE` 为 `auto` 或 `on`；`LTBASE_GOVERNANCE_ACTION_MODE=on` 还额外要求 `LTBASE_GOVERNANCE_MODE`、`LTBASE_LTFLOW_MODE` 和 `LTBASE_SEMANTIC_MODE` 为 `auto` 或 `on`。违反依赖链时，bootstrap 和 `pulumi up` 会直接报错失败。
    - `FORMA_CDC_S3_PREFIX`（默认不设置）：runtime bucket 下的 CDC 输出前缀。`LTBASE_CDC_MODE=on` 时部署会自动默认为 `delta`；`auto` 模式下必须显式设置该值才能启用 CDC。
    - 迁移说明：模板总是会把 `LTBASE_GOVERNANCE_ACTION_MODE` 写入每个 stack，因此旧的 `GOVERNANCE_ACTION_ENGINE_ENABLED=true` 开关在 managed 部署中不再生效 — 请改用 `LTBASE_GOVERNANCE_ACTION_MODE=on`。
...
- `LTBASE_LTSEARCH_MODE`、`LTBASE_CDC_MODE`、`LTBASE_LTFLOW_MODE`、`LTBASE_SEMANTIC_MODE`、`LTBASE_GOVERNANCE_MODE`、`LTBASE_GOVERNANCE_ACTION_MODE`
  - 默认值：除 `LTBASE_GOVERNANCE_ACTION_MODE` 默认为 `off` 外，其余均为 `auto`
- `FORMA_CDC_S3_PREFIX`
  - 默认值：不设置；`LTBASE_CDC_MODE=on` 时部署自动默认为 `delta`
- `PULUMI_BACKEND_URL`
...
- capability mode override 只能填写 `auto`、`on` 或 `off`；只有当你希望缺少依赖时 Lambda 启动直接失败，而不是自动降级时，才使用 `on`
- capability mode 之间存在依赖链，bootstrap 和 `pulumi up` 会强制校验：`LTBASE_GOVERNANCE_MODE=on` 要求 `LTBASE_SEMANTIC_MODE` 为 `auto` 或 `on`；`LTBASE_GOVERNANCE_ACTION_MODE=on` 要求 `LTBASE_GOVERNANCE_MODE`、`LTBASE_LTFLOW_MODE` 和 `LTBASE_SEMANTIC_MODE` 为 `auto` 或 `on`
- `LTBASE_CDC_MODE=on` 时部署会把 `FORMA_CDC_S3_PREFIX` 自动默认为 `delta`；`auto` 模式下需在 `.env` 中显式设置 `FORMA_CDC_S3_PREFIX` 才能启用 CDC（重新运行 bootstrap 会从 `.env` 同步该值，`.env` 是唯一事实来源）
- 旧的 `GOVERNANCE_ACTION_ENGINE_ENABLED=true` 开关在 managed 部署中不再生效，因为模板总是会写入 `LTBASE_GOVERNANCE_ACTION_MODE`；请改用 `LTBASE_GOVERNANCE_ACTION_MODE=on`
