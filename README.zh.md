...
当前 bootstrap 流程还会管理 OIDC discovery Cloudflare Pages 项目（以直接上传方式创建，无 companion 仓库）、其自定义域名绑定、指向 `${OIDC_DISCOVERY_PAGES_PROJECT}.pages.dev` 的 zone 级 `CNAME`，以及每个 stack 的只读 discovery role。discovery 文档本身则由 `rollout-hop.yml` 在每个 rollout hop 中自动发布（rollout 之前用占位 key 发布，使 API Gateway authorizer 可以创建；rollout 之后再用真实的 KMS key 重新发布），复用共享的 `.github/actions/publish-oidc-discovery` composite action。`publish-oidc-discovery.yml` 工作流仍保留为手动恢复 / 重新发布入口。
