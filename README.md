...
The bootstrap flow also manages the OIDC discovery Cloudflare Pages project (created as a direct-upload project, no companion repository), its custom domain binding, the required zone-level CNAME pointing at `${OIDC_DISCOVERY_PAGES_PROJECT}.pages.dev`, and the per-stack read-only discovery roles. The discovery documents themselves are then published automatically by `rollout-hop.yml` on every rollout hop (before rollout with a placeholder key so the API Gateway authorizer can be created, and again after rollout with the real KMS-backed key), using the shared `.github/actions/publish-oidc-discovery` composite action. The `publish-oidc-discovery.yml` workflow remains available as a manual recovery / re-publish entry point.
